...
class SecureMail
{
	...
	const SERVERADDR		= 'server <server@mystryl.art>'; // Server e-mail address.
	const DEFAULTTO			= 'info@mystryl.art'; // default "to" e-mail address when address has not been provided.
	...
	*/		
	public function clean($string,$method) {
		...
		switch($method) {
			...
			break;
			case 'domain':
				$this->data =  str_ireplace(array('https://','www.'),array('',''),$string);
			break;				
			case 'body':
